...
# Set kubelet register taints
KUBELET_REGISTER_WITH_TAINTS=${kubelet_register_with_taints}
if [[ $KUBELET_REGISTER_WITH_TAINTS != "" ]]; then sed -i '/INTERNAL_IP/a \ \ --register-with-taints='"$KUBELET_REGISTER_WITH_TAINTS"'\ \\' /etc/systemd/system/kubelet.service; fi
